...
class ml_Meta_Box {
	...
	function check_field_upload() {
		...
		add_action('wp_ajax_ml_delete_file', array(&$this, 'delete_file'));			// ajax delete files
		add_action('wp_ajax_ml_reorder_images', array(&$this, 'reorder_images'));	// ajax reorder images
	}
	...
	function insert_images() {
		if (!isset($_POST['ml-insert']) || empty($_POST['attachments'])) return;

		check_admin_referer('media-form');

		$nonce = wp_create_nonce('ml_ajax_delete');
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image','meydjer') . "' class='ml-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete','meydjer') . "</a>";
	...
	function delete_file() {
		...
		if (!wp_verify_nonce($nonce, 'ml_ajax_delete')) die('1');
	...
	function reorder_images() {
		...
		if (!wp_verify_nonce($nonce, 'ml_ajax_reorder')) die('1');
	...
	function check_field_date() {
		if ($this->has_field('date') && self::is_edit_page()) {
			...
			wp_enqueue_style('ml-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('ml-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && self::is_edit_page()) {
			...
			wp_enqueue_style('ml-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('ml-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('ml-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('ml-jquery-ui'));
		}
	}
	...
	function show_field_begin($field, $meta) {
		echo "<th class='ml-label'><label for='{$field['id']}'>{$field['name']}</label></th><td class='ml-field'>";
	}
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='ml-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' style='{$field['style']}' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='ml-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select id=".$field['id']." class='ml-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . " >";
	...
	function show_field_radio($field, $meta) {
		$this->show_field_begin($field, $meta);
		foreach ($field['options'] as $key => $value) {
			echo "<input type='radio' class='ml-radio' name='{$field['id']}' value='$key'" . checked($meta, $key, false) . " /> $value ";
		}
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='checkbox' class='ml-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked(!empty($meta), true, false) . " /> {$field['desc']}</td>";
	}

	function show_field_wysiwyg($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='ml-wysiwyg theEditor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			$nonce = wp_create_nonce('ml_ajax_delete');
			echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files','meydjer') . '</strong></div>';
			echo '<ol class="ml-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='ml-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete','meydjer') . "</a>)</li>";
			}
			echo '</ol>';
		}

		// show form upload
		echo "<div style='clear: both'><strong>" . __('Upload new files','meydjer') . "</strong></div>
			<div class='new-files'>
				<div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
				<a class='ml-add-file' href='#'>" . __('Add more file','meydjer') . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image($field, $meta) {
		...
		$nonce_delete = wp_create_nonce('ml_ajax_delete');
		$nonce_sort = wp_create_nonce('ml_ajax_reorder');

		echo "<input type='hidden' class='ml-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='ml-images ml-upload' id='ml-images-{$field['id']}'>";
		...
		if (!empty($meta)) {
			...
			foreach ($images as $image) {
				...
				echo "<li id='item_$image'>
						<img src='$src' />
						<a title='" . __('Delete this image','meydjer') . "' class='ml-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete','meydjer') . "</a>
		...
		echo "<a href='#' class='ml-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images','meydjer') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='ml-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='ml-color-select' rel='{$field['id']}'>" . __('Select a color','meydjer') . "</a>
			  <div style='display:none' class='ml-color-picker' rel='{$field['id']}'></div>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='ml-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='ml-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='ml-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_upload($field, $meta) {
		global $post;
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='upload' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		echo "<input type='button' class='upload_button button' id='upload_{$field['id']}' value='" . __('Upload', 'meydjer') . "' rel='".$post->ID."' />";
		$this->show_field_end($field, $meta);
	}
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'ml_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('ml_Meta_Box_Validate') && method_exists('ml_Meta_Box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('ml_Meta_Box_Validate', $field['validate_func']), $new);
			}
...
function my_admin_scripts() {
	wp_register_script('ml_post_edit', get_template_directory_uri() . '/js/ml_post_edit.js.php', 'jquery', '1.0');

	wp_enqueue_script('thickbox');
	wp_enqueue_script( 'of-medialibrary-uploader' );
	wp_enqueue_script( 'media-upload' );
	wp_enqueue_script( 'ml_post_edit' );
}
