...
/*-----------------------------------------------------------------------------------*/
	...
	var formfield,
		...
		//Change "insert into post" to "OK"
		tbframe_interval = setInterval(function() {jQuery('#TB_iframeContent').contents().find('.savesend .button').val('OK');}, 2000);
        
        // Display a custom title for each Thickbox popup.
        var woo_title = '';
        
		if ( jQuery(this).parents('.section').find('.heading') ) { woo_title = jQuery(this).parents('.section').find('.heading').text(); } // End IF Statement
	...
	window.send_to_editor = function(html) {
		...
		if (formfield) {
			...
			// itemurl = jQuery(html).attr('href'); // Use the URL to the main image.
