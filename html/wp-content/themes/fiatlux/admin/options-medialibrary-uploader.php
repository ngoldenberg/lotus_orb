...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) {
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels' => array(
				'name' => __( 'Options Framework Internal Container','meydjer' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'optionsframework_mlu_js' ) ) {
	...
	function optionsframework_mlu_js () {
		...
		// Enqueues custom scripts for the Media Library AJAX uploader.
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload','meydjer' ) . '" rel="' . $int . '" />' . "\n";
...
?>
	...
	jQuery(function($) {
		...
		jQuery( '.savesend a.del-link' ).click ( function () {
		
			var continueButton = jQuery( this ).next( '.del-attachment' ).children( 'a.button[id*="del"]' );
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'optionsframework' ), __( 'Previously Uploaded', 'meydjer' ), $tabs['gallery'] );
		return $tabs;
	}
}